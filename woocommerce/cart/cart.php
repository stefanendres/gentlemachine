			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
						...
						?>
						<?php if (count(get_product_tags($product_id)) > 0) : ?>
							<div class="product-tags">
							<?php foreach (get_product_tags($product_id) as $product_tag): ?>
						...
						<td class="product-price" data-title="<?php esc_attr_e( 'Price', 'woocommerce' ); ?>">
							<?php if (has_term('change', 'product_tag', $product_id)): ?>
								<div class="product-price-change">
									<?= get_field('coin_price', $product_id) ?> <?= get_field('coin_currency_text', 'options') ?>
								</div>
    						<?php else: ?>
						...
						<td class="product-subtotal" data-title="<?php esc_attr_e( 'Subtotal', 'woocommerce' ); ?>">
							<?php if (has_term('change', 'product_tag', $product_id)): ?>
								<div class="product-price-change">
									<?= $cart_item['quantity'] * get_field('coin_price', $product_id) ?> <?= get_field('coin_currency_text', 'options') ?>
								</div>
    						<?php else: ?>
