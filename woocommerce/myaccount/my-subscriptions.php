	...
	<?php else : ?>
		<p class="no_subscriptions woocommerce-message woocommerce-message--info woocommerce-Message woocommerce-Message--info woocommerce-info">
			<?php if ( 1 < $current_page ) :
				...
				?>
				<a class="woocommerce-Button button" href="<?= get_term_link(get_term_by('slug', 'subscription', 'product_cat')->term_id, 'product_cat') ?>">
					<?php
						// esc_html_e( 'Browse subscriptions', 'woocommerce-subscriptions' ); 
						$aboshop_title_url = wp_get_attachment_image_src(get_field('aboshop_name_svg', 'options'))[0];
					?>
					<!--<img class="custom-title-svg" src="<?= $aboshop_title_url ?>" alt="Visit Aboshop"/>-->
					Go to Aboshop
				</a>
			<?php
		endif; ?>
