    ...
    <?php if (is_user_logged_in()): ?>
      <?php foreach ($product_tags as $product_tag): ?>
        <div class="product-tag product-tag-<?= $product_tag['slug'] ?>">
          <img class="icon" src="<?= $product_tag['icon_url'] ?>" alt="<?= gm_get_context()['site_title'] ?> – <?= $product_tag['name']; ?> Icon"/>
          <span class="name"><?= $product_tag['name']; ?></span>
        </div>
      <?php endforeach ?>
    <?php else: ?>
      <?php foreach ($product_tags as $product_tag): ?>
        <a class="product-tag-signup-link product-tag-<?= $product_tag['slug'] ?>" href="<?= get_permalink(get_option('woocommerce_myaccount_page_id')) ?>">
          <img class="icon" src="<?= $product_tag['icon_url'] ?>" alt="<?= gm_get_context()['site_title'] ?> – <?= $product_tag['name']; ?> Icon"/>
          <span class="name">Sign up to <?= $product_tag['name']; ?></span>
        </a>
      <?php endforeach ?>
    <?php endif ?>
  </div>
...
function gm_after_single_product_summary() {
  ?>
  <a class="single-product-back-link" href="<?= get_permalink(wc_get_page_id('shop')); ?>">Shop</a>
  <?php
}
